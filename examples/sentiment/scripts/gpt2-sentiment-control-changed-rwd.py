...
config = PPOConfig(
    ...
    log_with="wandb",
    tracker_kwargs={
        "init_kwargs": {"wandb":{"name":"logit_diff_reward"}}
    }
)
...
def extract_pipe_output(outputs):
    '''outputs (negative_logit, positive_logit)'''
    flat_output = [{item['label'] : item['score'] for item in e} for e in outputs]
    return torch.tensor([[e['NEGATIVE'], e['POSITIVE']] for e in flat_output], dtype=torch.float32)
...
def logits_to_reward(logits, task):
    ...
    Take the positive sentiment logit and scale it for the task.
        task [negative]: reward = -(pos_logit - neg_logit)*0.5
        task [neutral]: reward = -abs(pos_logit - neg_logit)+4
        task [positive]: reward = (pos_logit - neg_logit)*0.5
    """
    rewards = []
    for i in range(len(logits)):
        if task[i]=='[negative]':
            rewards.append(0.5*(logits[i][0] - logits[i][1]))
        elif task[i]=='[neutral]':
            rewards.append(-0.5*torch.abs(logits[i][0] - logits[i][1]) + 4)
        elif task[i]=='[positive]':
            rewards.append(0.5*(logits[i][1] - logits[i][0]))
        else:
