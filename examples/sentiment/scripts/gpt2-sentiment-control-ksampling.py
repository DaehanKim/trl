...
from trl import PPOTrainerSample, PPOConfig, AutoModelForCausalLMWithValueHead, create_reference_model
...
config = PPOConfig(
    ...
    log_with="wandb",
    tracker_kwargs={
        "init_kwargs": {"wandb":{"name":"original+top10sampling"}}
    }
)
...
ppo_trainer = PPOTrainerSample(config, gpt2_model, gpt2_model_ref, gpt2_tokenizer, dataset, data_collator=collator)
...
for epoch in range(4):
    for batch in tqdm(ppo_trainer.dataloader):
        ...
        #### Run PPO training 
        t = time.time()
        stats = ppo_trainer.step(query_tensors, response_tensors, rewards, generation_kwargs=generation_kwargs)
