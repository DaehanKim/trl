...
from .utils import TopPLogitsWarper, TopKLogitsWarper
...
class PPOTrainerSample(BaseTrainer):
    """
    ...
    def generate(self, query_tensor: torch.Tensor, **generation_kwargs):
        ...
        """
        self.generation_kwargs = generation_kwargs # setup generation keyword arguments for later logit computation
    ...
    def step(
        ...
        logprobs, ref_logprobs, values = self.batched_forward_pass(queries, responses, generation_kwargs=generation_kwargs)
    ...
    def batched_forward_pass(
        ...
        logit_warper = TopPLogitsWarper(top_p=1.0) # this processor does nothing
        if generation_kwargs is not None:
            # setup logit post-processor (top-p or top-k)
            assert not (generation_kwargs['top_k'] > 0 and generation_kwargs['top_p'] < 1.0), "either top-p or top-k is supported, not both."
            if generation_kwargs['top_k'] > 0:
                logit_warper = TopKLogitsWarper(top_k=generation_kwargs['top_k'])
            else:
                logit_warper = TopPLogitsWarper(top_p=generation_kwargs['top_p'], min_tokens_to_keep=2)
        ...
        for i in range(int(bs / fbs)):
            ...
            # irrelevant logits are masked out here before computing log probs
            logits = logit_warper(input_ids, logits) 
            ref_logits = logit_warper(input_ids, ref_logits) 
